...
*/   
    ...
    struct GameOptions {
        ...
        map<int, int> where_c_id;

        vector<SolarPanel> here[kMaxCountry];
        vector<int> next_thing[kMaxCountry];

        GameOptions(vector<int>& budget) : budget(budget) { 
            for (int i = 0; i < num_countries; i += 1) {
                for (auto& itr : good_cells[i]) {
                    next_thing[i].push_back(itr.first);
                }

                sort(next_thing[i].begin(), next_thing[i].end());
            }
        }

        bool Add(SolarPanel a, int countr_id) {

            if (where_c_id.count(a.uid)) { return false; }
            ...
            where_c_id[a.uid] = countr_id;
            here[countr_id].push_back(a);
            budget[countr_id] -= cost;
            return true;
        }

        void Erase(int countr_id) {
            if (here[countr_id].size() == 0) { return; }
            int index = rand() % here[countr_id].size();
            int uid = here[countr_id][index].uid;
            here[countr_id][index] = here[countr_id].back();
            here[countr_id].pop_back();
            
            int cost = Distance(solar_pannels[uid].id, countr_id) + solar_pannels[uid].cost;
            budget[countr_id] += cost;

            where_c_id.erase(uid);
        }

        int Peek(int c) {
            if (next_thing[c].size() == here[c].size()) { return 0;
            }
            return next_thing[c][here[c].size()];
        }
    ...
    while (1) {
        ...
        sort(countries.begin(), countries.end(), [&](auto a, auto b) {
            return go.Peek(a) * go.budget[a] > go.Peek(b) * go.budget[b]; });
        ...
        for (auto i : countries) {
            ...
            sort(all_solar_panels.begin(), all_solar_panels.end(), [&](auto a, auto b) {
                ...
                return 1.0 * a.bonus * a.bonus / c1 > 1.0 * b.bonus * b.bonus / c2;
            });
