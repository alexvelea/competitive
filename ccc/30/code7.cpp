...
struct Allien {
    Allien(int id, int spawn_time, int x, int y, double health, double speed, double slow_resistance, double aoe_resistance) //
          : id(id), spawn_time(spawn_time), x(x), y(y), health(health), speed(speed), slow_resistance(slow_resistance), aoe_resistance(aoe_resistance) {
              is_dead = false;
    }
    ...
    double health;
    double speed;
    double slow_resistance;
    double aoe_resistance;
    ...
    double Distance(const Allien& a) const {
        return (x - a.x) * (x - a.x) + (y - a.y) * (y - a.y) - 1e-7;
    }

    void ApplyTowerHit(const Tower& t, bool first_target);
};
...
struct Tower {
    ...
    bool operator<(const Tower&) const { return false; }
};


void Allien::ApplyTowerHit(const Tower& t, bool first_target) {
    if (first_target) {
        health -= t.damage;
    } else {
        health -= (1.0 - aoe_resist) * t.damage;
    }

    slow *= (1.0 * slow_resistance) * (1.0 - t.tt.slow);
}
...
int main(int argc, char**) {
    ...
    int num_allien_types = 0; cin >> num_allien_types;
    struct AllienType {
        string name;
        double health, speed, slow_resistance, aoe_resistance, loot;
    };
    
    map<string, AllienType> lookup;

    while (num_allien_types--) {
        string a;
        double b,c,d,e; cin >> a >> b >> c >> d >> e;
        int f; cin >> f;
        lookup[a] = AllienType({"", b, c, d, e, f});
    }
